...
| **delegationbench** | delegation: complexity decision (free) + live delegation precision/recall + fan-in fidelity | SDK-concept | `agent_sdk/subagents/` + `plugins/metacognition/path.py` + fan-out in `agent_sdk/engine.py` | **new** → `benchmarks/delegationbench/` (free + live, runnable) |
