...
| **delegationbench** | delegation: live delegation precision/recall + fan-in fidelity (per-subagent timelines in the report) | SDK-concept | `agent_sdk/plugins/subagents/` + fan-out in `agent_sdk/engine.py` | **new** → `benchmarks/delegationbench/` (live-only, runnable) |
| **promptbench** | prompt structure (layer order, no duplication) + quality (rule-based lint) + LLM-judge rubric | SDK-concept | `agent_sdk/engine.py` (`_compose_system_segmented`, `_PROMPT_LAYERS`) + authored prompt constants | **new** → `benchmarks/promptbench/` (free + live, runnable) |
