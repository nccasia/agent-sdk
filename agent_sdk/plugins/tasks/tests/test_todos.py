...
async def test_add_then_advance_to_complete():
    ...
    await rt.call_tool(
        "todos", {"action": "done", "result": "did A"}, [], set()
    )  # no id → next open
...
async def test_dependency_order_enforced_by_tool():
    ...
    await rt.call_tool("todos", {"action": "add", "title": "A"}, [], set())
    await rt.call_tool(
        "todos", {"action": "add_many", "steps": [{"title": "B", "deps": ["t0"]}]}, [], set()
    )
...
async def test_request_human_blocks():
    ...
    out = await rt.call_tool(
        "todos", {"action": "request_human", "question": "approve?"}, [], set()
    )
    assert (
        "Escalated" in out
        and rail.by_id("t0").status == "blocked"
        and rt.human_asks == ["approve?"]
    )
