...
from agent_sdk.lobes.skill._common import active_skill_packs, skill_flags
...
class SkillActiveLobe(Lobe):
    ...
    def _produce_guide(self, ctx: TurnContext) -> list[ContextNode]:
        ...
        return [ContextNode(id="skill:guide", kind="skill_guide", text=(
            f"A skill is in use{label}. Drive the task through the skill's content: "
            "execute its remaining steps in order, read referenced files/sections "
            "only as each step needs them, and continue until every step is complete "
            "before giving the final answer."), menu_hint="skill workflow in progress")]
